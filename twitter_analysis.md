...
<!-- # get_retweeters("1139476078395875328") -->

<!-- # tweets_with_rts[1:10] %>%  -->

<!-- #   set_names(tweets_with_rts[1:10]) %>%  -->

<!-- #   imap(~get_retweeters(.x)) -->

<!-- retweets <- vector(mode = 'list', length = length(tweets_with_rts)) -->

<!-- for (i in seq_along(tweets_with_rts)) { -->

<!--   message("Getting retweets for tweet #", i, "/9891") -->

<!--   retweets[[i]] <- get_retweeters(as.character(tweets_with_rts[i])) -->

<!--   # Sleep every 50 to let the rate limit reset -->

<!--   if(i %% 50 == 0) Sys.sleep(15*60)  -->

<!-- } -->

<!-- make_df_from_retweets <- function(retweets, tweet) { -->

<!--   if (nrow(retweets) > 0) { -->

<!--     tibble(from_id = retweets[[1]], -->
...
<!-- tweets_for_joining <- tweets %>%  -->

<!--   mutate(tweet_id = str_extract(url, '[0-9]+$')) -->

<!-- retweet_edges <- map2(retweets, tweets_with_rts, make_df_from_retweets) %>%  -->

<!--   bind_rows %>%  -->

<!--   left_join(tweets_for_joining, by = c("tweet_id")) %>%  -->

<!--   select(from_id, user) %>% -->

<!--   rename(to_name = user) %>%  -->

<!--   left_join(user_details, by = c('from_id' = 'user_id')) %>%  -->

<!--   select(screen_name, to_name) %>%  -->

<!--   rename(from_name = screen_name) %>%  -->

<!--   filter(!is.na(to_name) & !is.na(from_name)) %>%  -->
